...
/// The orchestrator prompt: each arm runs ONE warm-cache session that delegates
/// a subagent per file, then writes a combined hardware summary.
const PROMPT: &str = "\
You are an orchestrator agent measuring a proxy. Inside the added directory \
there are three files: drivers.txt, devices.txt, and system.txt.\n\
For EACH of the three files, launch a SEPARATE subagent (use your Task tool) whose job is to read \
that one file in full and report the three most notable hardware facts it contains. Do NOT read \
the files yourself -- delegate each file to its own subagent. Launch all three subagents.\n\
After all three subagents return, write a single combined hardware summary as EXACTLY 10 numbered \
sentences (1 to 10). No markdown, no headings, no code blocks, no lists. Then stop.";
...
/// Create the frozen corpus directory and populate it with system dumps.
/// Returns the corpus path and the list of (filename, bytes) pairs, or an error string.
pub fn snapshot_corpus() -> Result<(PathBuf, Vec<(String, usize)>), String> {
    ...
    let mut got_drivers = false;
    let mut got_devices = false;

    // driverquery /v /fo list  -> drivers.txt  (~217 KB)
    let drivers_path = corpus_dir.join("drivers.txt");
    if let Some(_content) = capture_command_to_file("driverquery", &["/v", "/fo", "list"], &drivers_path) {
        got_drivers = true;
        let bytes = std::fs::metadata(&drivers_path).map(|m| m.len()).unwrap_or(0) as usize;
        files.push(("drivers.txt".to_string(), bytes));
    }

    // pnputil /enum-devices  -> devices.txt  (~125 KB)
    let devices_path = corpus_dir.join("devices.txt");
    if let Some(_content) = capture_command_to_file("pnputil", &["/enum-devices"], &devices_path) {
        got_devices = true;
        let bytes = std::fs::metadata(&devices_path).map(|m| m.len()).unwrap_or(0) as usize;
        files.push(("devices.txt".to_string(), bytes));
    }

    // systeminfo  -> system.txt  (~5 KB)
    let system_path = corpus_dir.join("system.txt");
    if let Some(_content) = capture_command_to_file("systeminfo", &[], &system_path) {
        let bytes = std::fs::metadata(&system_path).map(|m| m.len()).unwrap_or(0) as usize;
        files.push(("system.txt".to_string(), bytes));
    }

    if !got_drivers && !got_devices {
        return Err(
            "Both drivers.txt and devices.txt are missing -- cannot proceed.\n\
             Make sure you are on Windows with driverquery and pnputil available."
                .to_string(),
        );
    }
...
pub fn run_measurement(
    ...
    if !names.contains(&"drivers.txt".to_string()) {
        set_msg(progress, "    drivers.txt  FAILED (driverquery not available or errored)");
    }
    if !names.contains(&"devices.txt".to_string()) {
        set_msg(progress, "    devices.txt  FAILED (pnputil not available or errored)");
    }
    if !names.contains(&"system.txt".to_string()) {
        set_msg(progress, "    system.txt  FAILED (systeminfo not available or errored; continuing)");
    }
