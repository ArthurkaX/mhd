...
/// A capacity below one whole token can never grant a permit: tokens are clamped
/// to capacity on every refill, so they never reach 1.0 and `acquire` would loop
/// forever. Floor every capacity at this value so the bucket always progresses.
...
impl TokenBucket {
    ...
    /// starts full. `capacity` is floored at [`MIN_CAPACITY`] so a misconfigured
    /// sub-token burst can't hang `acquire` forever.
    ...
    /// `capacity` is floored at [`MIN_CAPACITY`] for the same reason as in `new`.
