...
use windows::Win32::Foundation::{COLORREF, HANDLE, HWND, LPARAM, LRESULT, POINT, RECT, SIZE,
    WAIT_OBJECT_0, WPARAM, CloseHandle};
...
use windows::Win32::UI::WindowsAndMessaging::{
    ...
    ULW_ALPHA, WM_KEYDOWN, WM_ACTIVATE, WM_MOUSEWHEEL,
        ...
        if res == WAIT_OBJECT_0 {
            ...
            if let Some(action) = st.action.take() {
                match action {
                    ...
                    ToolbarAction::Clear => {
                        if st.history.len() >= 10 { st.history.remove(0); }
        ...
        match msg {
            ...
            WM_KEYDOWN => {
                ...
                if vk == 0x1B { // VK_ESCAPE
                    ...
                    let ctrl = unsafe { windows::Win32::UI::Input::KeyboardAndMouse::GetAsyncKeyState(windows::Win32::UI::Input::KeyboardAndMouse::VK_CONTROL.0 as i32) } as u16 & 0x8000;
                    if ctrl != 0 {
            ...
            WM_LBUTTONDOWN => {
                ...
                if x >= 0 && x < st.width && y >= 0 && y < st.height {
                    ...
                    match st.tool {
                        Tool::Pencil => {
                            fill_circle(&mut st.pixels, st.width, st.height, x, y, st.thickness, argb_pixel_from_idx(st.color_idx));
                            st.drag = Some((x, y));
                        }
                        Tool::Rect | Tool::Arrow => {
                            st.backup.copy_from_slice(&st.pixels);
                            let color = argb_pixel_from_idx(st.color_idx);
                            draw_rect_outline(&mut st.pixels, st.width, st.height, x, y, x, y, color, st.thickness);
                            st.drag = Some((x, y));
                        }
                        Tool::Circle => {
                            st.backup.copy_from_slice(&st.pixels);
                            let color = argb_pixel_from_idx(st.color_idx);
                            draw_circle_outline(&mut st.pixels, st.width, st.height, x, y, x, y, color, st.thickness);
                            st.drag = Some((x, y));
                        }
                    }
            ...
            WM_MOUSEMOVE => {
                ...
                if let Some((sx, sy)) = st.drag {
                    match st.tool {
                        Tool::Pencil => {
                            draw_thick_line(&mut st.pixels, st.width, st.height, sx, sy, x, y, argb_pixel_from_idx(st.color_idx), st.thickness);
                            st.drag = Some((x, y));
                            paint(hwnd, st, s);
                        }
                        Tool::Rect => {
                            st.pixels.copy_from_slice(&st.backup);
                            draw_rect_outline(&mut st.pixels, st.width, st.height, sx, sy, x, y, argb_pixel_from_idx(st.color_idx), st.thickness);
                            paint(hwnd, st, s);
                        }
                        Tool::Arrow => {
                            st.pixels.copy_from_slice(&st.backup);
                            draw_arrow(&mut st.pixels, st.width, st.height, sx, sy, x, y, argb_pixel_from_idx(st.color_idx), st.thickness);
                            paint(hwnd, st, s);
                        }
                        Tool::Circle => {
                            st.pixels.copy_from_slice(&st.backup);
                            draw_circle_outline(&mut st.pixels, st.width, st.height, sx, sy, x, y, argb_pixel_from_idx(st.color_idx), st.thickness);
                            paint(hwnd, st, s);
                        }
                    }
                }
...
fn hit_toolbar(x: i32, s: f32) -> Option<ToolbarAction> {
    ...
    // Tools
    for ti in 0..4 {
        if x >= left && x < left + tbw { return Some(ToolbarAction::SetTool(match ti { 0 => Tool::Pencil, 1 => Tool::Rect, 2 => Tool::Arrow, _ => Tool::Circle })); }
        left += tbw + gap;
    }
    ...
    // Actions
    if x >= left && x < left + abw { return Some(ToolbarAction::Undo); }
    left += abw + gap;
...
fn paint(hwnd: HWND, st: &State, s: f32) {
    ...
    for ti in 0..4 {
        let sel = match ti { 0 => st.tool == Tool::Pencil, 1 => st.tool == Tool::Rect, 2 => st.tool == Tool::Arrow, _ => st.tool == Tool::Circle };
        if sel {
...
fn draw_thick_line(pixels: &mut [u32], w: i32, h: i32, x0: i32, y0: i32, x1: i32, y1: i32, color: u32, radius: i32) {
    let dx = (x1 - x0).abs();
    let dy = -(y1 - y0).abs();
...
fn draw_rect_outline(pixels: &mut [u32], w: i32, h: i32, x0: i32, y0: i32, x1: i32, y1: i32, color: u32, radius: i32) {
    let x_min = x0.min(x1);
    let x_max = x0.max(x1);
    let y_min = y0.min(y1);
    let y_max = y0.max(y1);
...
fn draw_arrow(pixels: &mut [u32], w: i32, h: i32, x0: i32, y0: i32, x1: i32, y1: i32, color: u32, radius: i32) {
...
fn draw_circle_outline(pixels: &mut [u32], w: i32, h: i32, cx: i32, cy: i32, x1: i32, y1: i32, color: u32, radius: i32) {
    let dx = x1 - cx;
    let dy = y1 - cy;
