...
use windows::Win32::Graphics::Gdi::{
    ...
    DeleteObject, DrawTextW, FillRect, GetMonitorInfoW, MONITOR_DEFAULTTONEAREST, MONITORINFO,
    MonitorFromPoint, SelectObject, SetBkMode, SetTextColor, TRANSPARENT,
};
...
use crate::constants::{
    VCP_AUDIO_VOLUME, VCP_BRIGHTNESS, VCP_CONTRAST, VCP_INPUT_SOURCE, WM_MOUSELEAVE,
};
                        ...
                        if msg.hwnd == hwnd {
                            match msg.message {
                                ...
                                WM_LBUTTONDOWN => {
                                    ...
                                    if let Some((mi, pi)) =
                                        hit_test_param_slider(&state, x, y_adj, panel_w, scale)
                                    {
                                    ...
                                    // Input source cycle
                                    if let Some((mi, pi)) =
                                        hit_test_input_source(&state, x, y_adj, panel_w, scale)
                                    {
                                ...
                                WM_MOUSEWHEEL => {
                                    ...
                                    if let Some((mi, pi)) =
                                        hit_test_slider_row(&state, pt.0, y_adj, panel_w, scale)
                                    {
...
fn detect_features(m: &PhysicalMonitorInfo) -> Vec<ParamInfo> {
    ...
    let supported_codes = m.capabilities().ok().and_then(|caps| {
...
fn try_query_brightness(m: &PhysicalMonitorInfo) -> Option<ParamKind> {
    m.get_brightness()
        .ok()
        .map(|(cur, min, max)| ParamKind::Brightness {
            current: cur,
            min,
            max,
        })
...
fn param_is_slider(param: &ParamInfo) -> bool {
    matches!(
        &param.kind,
        ParamKind::Brightness { .. } | ParamKind::Contrast { .. } | ParamKind::AudioVolume { .. }
    )
}
...
fn param_display_value(param: &ParamInfo) -> String {
    match &param.kind {
        ...
        ParamKind::InputSource { current, values } => values
            .iter()
...
fn paint_panel(hwnd: HWND, state: &mut PanelState, work: &RECT, width: i32, scale: f32) {
        ...
        for param in monitor.params.iter() {
            ...
            if row_y + row_h >= clip_top && row_y < clip_bot {
                if param_is_slider(param) {
                    ...
                    // Bar track
                    let track_brush =
                        unsafe { CreateSolidBrush(theme.bar_background.to_colorref()) };
                    let track_rc = RECT {
                        left: bar_x,
                        top: bar_y,
    ...
    if state.scroll.max > 0 {
        ...
        let thumb_pos =
            (state.scroll.y as f32 / state.scroll.max as f32 * (sb_height - thumb_h) as f32) as i32;
        let thumb_y = pad + thumb_pos;

        let sb_color = crate::native_theme::Argb {
            a: 80,
    ...
    for (mi, monitor) in state.monitors.iter().enumerate() {
        ...
        for (pi, param) in monitor.params.iter().enumerate() {
            if !param_is_slider(param) {
                ...
                if y >= current_y && y < current_y + row_h && x >= pill_x && x < pill_right {
                    return Some((mi, pi));
                }
            }
            current_y += row_h;
...
fn set_param_value(state: &mut PanelState, mi: usize, pi: usize, ratio: f32) {
    let Some(monitor) = state.monitors.get(mi) else {
        return;
    };
    let Some(param) = monitor.params.get(pi) else {
        return;
    };
