//! mhd-inspector — GUI to inspect one captured request and its native-trim before/after.
...
fn extract<'a>(root: &'a Value, path: &NodePath) -> Option<&'a Value> {
...
fn build_tree(before: &Value, after: &Value) -> Vec<Node> {
    ...
    if let Some(tools) = before.get("tools").and_then(|v| v.as_array()) {
        ...
        for (i, tool) in tools.iter().enumerate() {
            // Anthropic native tools are flat ({"name", "description", ...}).
            // Fall back to the OpenAI-shaped `function.name` just in case.
            let name = tool
                .get("name")
                .and_then(|n| n.as_str())
    ...
    if let Some(msgs) = before.get("messages").and_then(|v| v.as_array()) {
        ...
        for (i, msg) in msgs.iter().enumerate() {
            ...
            // Check for tool_result grandchildren
...
fn load_settings_from_disk(knobs: &mut NativeKnobs) {
...
struct InspectorApp {
    ...
    /// Window-subtitle fields
    run_id: i64,
    seq: i64,
    model: String,
    dirty: bool,
    /// DB path kept so navigation can re-open read-only.
    db_path: PathBuf,
    /// Rowid of the currently loaded request (for Prev/Next stepping).
    row_id: Option<i64>,
}
...
impl InspectorApp {
    fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        let result = if let Some(id) = cli.row_id {
            query_by_id(&conn, id)
        } else if let (Some(rid), Some(s)) = (cli.run_id, cli.seq) {
    ...
    /// Load a fetched row: set metadata, decompress + parse the body, run trim,
    /// rebuild the tree. Resets the selected diff node.
    fn apply_row(&mut self, row: RequestRow) {
    ...
    /// Step to the adjacent captured request by rowid. `dir` is -1 (prev/older)
    /// or +1 (next/newer). No-op if there is no current row or no neighbor.
    fn navigate(&mut self, dir: i64) {
...
fn query_by_id(conn: &rusqlite::Connection, id: i64) -> Result<Option<RequestRow>, rusqlite::Error> {
...
/// Fetch the neighboring captured request by rowid. `dir < 0` = the next-lower
/// id (older), `dir >= 0` = the next-higher id (newer). Steps across all
/// providers so navigation reaches every captured body.
fn query_neighbor(
    conn: &rusqlite::Connection,
    cur: i64,
    dir: i64,
...
// ── Context grid (Claude Code /context style) ────────────────────────────────

/// Category palette for the composition grid.
const COL_SYSTEM: egui::Color32 = egui::Color32::from_rgb(100, 150, 240);
const COL_TOOLS: egui::Color32 = egui::Color32::from_rgb(240, 180, 80);
const COL_MESSAGES: egui::Color32 = egui::Color32::from_rgb(80, 200, 140);
/// Empty cells in the "after" grid = tokens the trim freed.
const COL_SAVED: egui::Color32 = egui::Color32::from_rgb(215, 90, 90);
/// Empty cells in the "before" grid (should be none, but for rounding).
const COL_EMPTY: egui::Color32 = egui::Color32::from_rgb(224, 224, 224);

/// Top-level category token counts for a request body: (system, tools, messages).
fn category_tokens(body: &Value) -> (u64, u64, u64) {
...
/// Paint a 10x10 composition grid. `denom` is the shared token scale for one
/// cell = denom/100. `segs` are (tokens, color) filled in order; cells past the
/// segment sum are painted `free_color`.
fn draw_grid(
    ui: &mut egui::Ui,
    denom: u64,
    ...
    for k in 0..CELLS {
        // Token position at the cell midpoint, in reading order.
...
fn show_tree(ui: &mut egui::Ui, nodes: &[Node], selected: &mut Option<NodePath>) {
    for node in nodes {
        ...
        // If no children, still show as a leaf (not a collapsing header)
        if node.children.is_empty() {
            ...
            header.show(ui, |ui| {
                // On the header line, show the label + right-aligned delta
                ui.horizontal(|ui| {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::SidePanel::left("knobs")
            ...
            .show(ctx, |ui| {
                ...
                // ── Context distribution matrix (moved under the knobs) ──
                if let (Some(before), Some(after)) = (&self.before, &self.after) {
                    ui.separator();
                    ...
                    // Compact colored legend + saved total.
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            // ── Navigation: step through captured requests ──
            let mut nav: i64 = 0;
            ui.horizontal(|ui| {
